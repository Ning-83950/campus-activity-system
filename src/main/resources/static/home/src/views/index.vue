<template>
	<div class="page_root" id="root_demo">
		    ...
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
						...
						<!-- 推荐活动信息模块(开始) -->
		<div class="ins warp model1" v-if="$check_action('/activity_information/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
							...
							<div class="text_t"><span></span></div>
		                    <div class="more_box">
		                        <router-link to="/activity_information/list" class="ins_more more">
		                ...
		                <!-- 活动信息推荐列表组件 -->
						<div class="list_activity_information_box" ins_box>
							<div class="Left_box">
								<span class="i"></span>
								<span class="i"></span>
								<span class="i"></span>
                        	</div>
							<list_activity_information :list="list_activity_information" />
							<div class="right_box">
								<span class="i"></span>
								<span class="i"></span>
								<span class="i"></span>
		...
		<div class="warp hot_rank" v-if="$check_action('/activity_information/list', 'get')">
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                <div class="diy_recommend">
						...
						<div class="hot_rank_list">
							<div class="hot_rank_item" v-for="(o, i) in list_hot_activity" :key="o.activity_information_id" @click="to_details('activity_information', o, 'activity_information_id')">
								<span class="rank_no">{{ i + 1 }}</span>
								<img :src="$fullUrl(o.activity_cover)" />
								<div class="rank_info">
									<strong>{{ o.event_name }}</strong>
									<p>{{ o.activity_type }} | {{ o.activity_time }}</p>
									<p>热度 {{ o.heat_score }} · 报名 {{ o.signup_count }} · 签到 {{ o.checkin_count }}</p>
								</div>
							</div>
						</div>
		            </div>
...
<script>
	import mixin from "@/mixins/page.js";
					import list_activity_information from "@/components/diy/list_activity_information.vue";
	...
	export default {
		...
		methods: {
			...
			get_activity_information() {
				let url = "~/api/activity_information/get_list?";
														let param = {
					...
					if (json.result) {
						this.list_activity_information = json.result.list;
					}
				})
			},

			get_hot_activity() {
				this.$get("~/api/activity_information/hot_rank?", { size: 8 }, (json) => {
					if (json.result && json.result.list) {
						this.list_hot_activity = json.result.list;
					}
				})
			},
		...
		mounted() {
							this.get_activity_information();
								this.get_hot_activity();
								this.get_menu();
			this.get_slides();
			this.get_article();
			this.get_notice();
		...
		computed: {
						    list_table_feedback_evaluation() {
		        var list = this.list_activity_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_activity_information.map((o) => {
		                // 第二个中括号是对象的属性
