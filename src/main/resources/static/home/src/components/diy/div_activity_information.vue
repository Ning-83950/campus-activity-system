<template>
	<div class="diy_details_box diy_div_activity_information">
		<div class="warp diy_warp">
			<div class="container">
				...
				<div class="row row_det">
						<div v-if="this.$store.state.user.user_id" class="btns_add col-12 col-sm-6">
							<div v-if="$check_action('/activity_information/details','set')">
								<!-- 点赞按钮 -->
								<div
										v-if="state_praise"
			...
			<div class="details_btn_wrap">
																											<button class="btn btn-primary details_btn" v-if="$check_action('/enrollment_information/edit','add') && !enrollment_information_limit" @click="to_form('/enrollment_information/edit')" ><span> 报名</span> </button>
																										</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				list_user_teacher_user: [],
																		state_praise:false,
				state_collect:false,
							enrollment_information_limit: false,
						tableData: [],
			};
		},
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "activity_information_id",
					source_id: this.obj["activity_information_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/activity_information/set?activity_information_id=' + _this.obj["activity_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
																		query.title = this.obj.event_name
																													query.img = this.obj.activity_cover
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var activity_information_id = this.obj.activity_information_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "activity_information",
          source_field: "activity_information_id",
          source_id: activity_information_id
        }, (res) => {
          this.state_collect = res.result
        });
      },
			  get_enrollment_information_limit(){
	  	let param = {
	  		source_table: "activity_information",
	  		source_id: this.obj.activity_information_id,
	  		source_user_id: this.user.user_id
	  	};
	  	if(this.obj.enrollment_information_limit_times > 0){
	  		this.$get("~/api/enrollment_information/count?",param,(result)=>{
	  			if(result){
	  				if(result.result >= this.obj.enrollment_information_limit_times){
	  					this.enrollment_information_limit = true;
	  				}else{
	  					this.enrollment_information_limit = false;
	  				}
	  			}
	  		})
	  	}else{
        ...
        this.get_praise();
        this.check_collect();
				this.get_enrollment_information_limit();
		      }, 1000);
		},
		watch:{
			obj:{
