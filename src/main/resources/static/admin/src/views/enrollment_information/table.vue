<template>
	...
	<el-row class="rows row2">
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/enrollment_information/table','set') || $check_action('/enrollment_information/view','set') || $check_action('/enrollment_information/view','get') 
				...
				<template slot-scope="scope">
					...
					v-if="$check_action('/enrollment_information/table','set') || $check_action('/enrollment_information/view','set') || $check_action('/enrollment_information/view','get')"
						...
						<el-button class="e-button el-button--small is-plain el-button--warning" style="margin: 5px !important;" size="small" @click="manualCheckin(scope.row)" v-if="scope.row['examine_state'] == '已通过' && !scope.row['checkin_record']">
						<span>签到</span>
					</el-button>
						<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="downloadCertificate(scope.row)" v-if="scope.row['checkin_record']">
						<span>证明</span>
					</el-button>
						<!--跨表按钮-->
										<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/activity_notification/view')" v-if="($check_action('/activity_notification/table','add') || $check_action('/activity_notification/view','add')) && scope.row['examine_state'] == '已通过' && !scope.row['activity_notification_limit']">
						<span>通知</span>
					</el-button>
													<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/activity_costs/view')" v-if="($check_action('/activity_costs/table','add') || $check_action('/activity_costs/view','add')) && scope.row['examine_state'] == '已通过' && !scope.row['activity_costs_limit']">
						<span>费用</span>
					</el-button>
													<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/feedback_evaluation/view')" v-if="($check_action('/feedback_evaluation/table','add') || $check_action('/feedback_evaluation/view','add')) && scope.row['examine_state'] == '已通过' && !scope.row['feedback_evaluation_limit']">
						<span>评价</span>
					</el-button>
											<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="changeSigning(scope.row, scope.$index)" v-if="$check_option('/enrollment_information/table', 'examine')">
						<span>审核</span>
					</el-button>
				</div>
				</template>
